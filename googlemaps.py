...
class GoogleMapsScraper:
    ...
    def __parse(self, review):
        ...
        try:
            response = self.__filter_string(review.find(
                'div', class_='section-review-owner-response').find('div', class_='section-review-text').text)
        except Exception as e:
            response = None

        try:
            hasResponse = self.__filter_string(review.find(
                'span', class_='section-review-owner-response-title').text)
        except Exception as e:
            hasResponse = None
        ...
        item['Post'] = review_text
        if hasResponse is None:
            item['IsResponded'] = 'False'
        else:
            item['IsResponded'] = 'True'
        item['Comments'] = response
        item['PostUrl'] = user_url
        return item
    ...
    def __parse_place(self, response):
        ...
        place = {}
        try:
            place['overall_rating'] = float(response.find(
                'div', class_='gm2-display-2').text.replace(',', '.'))
        except:
            place['overall_rating'] = 'NOT FOUND'

        try:
            place['n_reviews'] = int(response.find(
                'div', class_='gm2-caption').text.replace('.', '').replace(',', '').split(' ')[0])
        except:
            place['n_reviews'] = 0
